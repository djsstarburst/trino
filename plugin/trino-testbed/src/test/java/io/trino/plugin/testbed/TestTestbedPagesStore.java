...
public class TestTestbedPagesStore
{
    private static final int POSITIONS_PER_PAGE = 0;

    private TestbedPagesStore pagesStore;
    private TestbedPageSinkProvider pageSinkProvider;

    @BeforeMethod
    public void setUp()
    {
        pagesStore = new TestbedPagesStore(new TestbedConfig().setMaxDataPerNode(DataSize.of(1, DataSize.Unit.MEGABYTE)));
        pageSinkProvider = new TestbedPageSinkProvider(pagesStore, HostAddress.fromString("localhost:8080"));
    }
    ...
    @Test(expectedExceptions = TrinoException.class)
    public void testTestbedLimitExceeded()
    {
        createTable(0L, 0L);
    ...
    private void insertToTable(long tableId, Page page, Long... activeTableIds)
    {
        ConnectorPageSink pageSink = pageSinkProvider.createPageSink(
                TestbedTransactionHandle.INSTANCE,
                SESSION,
                createTestbedInsertTableHandle(tableId, activeTableIds));
    ...
    private void createTable(long tableId, Long... activeTableIds)
    {
        ConnectorPageSink pageSink = pageSinkProvider.createPageSink(
                TestbedTransactionHandle.INSTANCE,
                SESSION,
                createTestbedOutputTableHandle(tableId, activeTableIds));
        pageSink.finish();
    }

    private static ConnectorOutputTableHandle createTestbedOutputTableHandle(long tableId, Long... activeTableIds)
    {
        return new TestbedOutputTableHandle(tableId, ImmutableSet.copyOf(activeTableIds));
    }

    private static ConnectorInsertTableHandle createTestbedInsertTableHandle(long tableId, Long[] activeTableIds)
    {
        return new TestbedInsertTableHandle(tableId, ImmutableSet.copyOf(activeTableIds));
    }
