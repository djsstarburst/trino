...
import static io.trino.plugin.testbed.TestbedColumnHandle.ROW_ID_HANDLE;
        ...
        implements ConnectorMetadata
{
    ...
    @Inject
    public TestbedMetadata(NodeManager nodeManager)
    {
    ...
    @Override
    public synchronized TestbedOutputTableHandle beginCreateTable(ConnectorSession session, ConnectorTableMetadata tableMetadata, Optional<ConnectorNewTableLayout> layout)
    {
        checkSchemaExists(tableMetadata.getTable().getSchemaName());
        checkTableNotExists(tableMetadata.getTable());
        long tableId = nextTableId.getAndIncrement();
        Set<Node> nodes = nodeManager.getRequiredWorkerNodes();
        checkState(!nodes.isEmpty(), "No Testbed nodes available");

        ImmutableList.Builder<ColumnInfo> columns = ImmutableList.builder();
        for (int i = 0; i < tableMetadata.getColumns().size(); i++) {
            ColumnMetadata column = tableMetadata.getColumns().get(i);
            columns.add(new ColumnInfo(new TestbedColumnHandle(i), column.getName(), column.getType()));
        }
        ...
        return new TestbedOutputTableHandle(tableId, ImmutableSet.copyOf(tableIds.values()));
    }
    ...
    public synchronized Optional<ConnectorOutputMetadata> finishCreateTable(ConnectorSession session, ConnectorOutputTableHandle tableHandle, Collection<Slice> fragments, Collection<ComputedStatistics> computedStatistics)
    {
        ...
        TestbedOutputTableHandle memoryOutputHandle = (TestbedOutputTableHandle) tableHandle;
    ...
    @Override
    public synchronized TestbedInsertTableHandle beginInsert(ConnectorSession session, ConnectorTableHandle tableHandle, List<ColumnHandle> columns)
    {
        TestbedTableHandle memoryTableHandle = (TestbedTableHandle) tableHandle;
        return new TestbedInsertTableHandle(memoryTableHandle.getId(), ImmutableSet.copyOf(tableIds.values()));
    }
    ...
    public synchronized Optional<ConnectorOutputMetadata> finishInsert(ConnectorSession session, ConnectorInsertTableHandle insertHandle, Collection<Slice> fragments, Collection<ComputedStatistics> computedStatistics)
    {
        ...
        TestbedInsertTableHandle memoryInsertHandle = (TestbedInsertTableHandle) insertHandle;
    ...
    {
        return ((TestbedTableHandle) tableHandle).withQueryType(QueryType.DELETE);
    }
    ...
    {
        return ((TestbedTableHandle) tableHandle).withQueryType(QueryType.UPDATE);
    }
    ...
    private void updateRowsOnHosts(long tableId, Collection<Slice> fragments)
    {
        ...
        Map<HostAddress, TestbedDataFragment> dataFragments = new HashMap<>(info.getDataFragments());
        for (Slice fragment : fragments) {
            TestbedDataFragment memoryDataFragment = TestbedDataFragment.fromSlice(fragment);
            dataFragments.merge(memoryDataFragment.getHostAddress(), memoryDataFragment, TestbedDataFragment::merge);
        }
    ...
    public List<TestbedDataFragment> getDataFragments(long tableId)
    {
    ...
    {
        TestbedTableHandle table = (TestbedTableHandle) handle;
        ...
        return Optional.of(new LimitApplicationResult<>(
                new TestbedTableHandle(table.getId(), OptionalLong.of(limit), OptionalDouble.empty(), table.getQueryType()),
                true));
    }
    ...
    public Optional<ConnectorTableHandle> applySample(ConnectorSession session, ConnectorTableHandle handle, SampleType sampleType, double sampleRatio)
    {
        ...
        return Optional.of(new TestbedTableHandle(table.getId(), table.getLimit(), OptionalDouble.of(table.getSampleRatio().orElse(1) * sampleRatio), table.getQueryType()));
    }
}
