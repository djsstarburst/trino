...
public final class TestbedPageSourceProvider
        implements ConnectorPageSourceProvider
{
    ...
    @Inject
    public TestbedPageSourceProvider(TestbedPagesStore pagesStore, TestbedConfig config)
    {
    ...
    {
        TestbedSplit memorySplit = (TestbedSplit) split;
        ...
        TestbedTableHandle memoryTable = (TestbedTableHandle) table;
        OptionalDouble sampleRatio = memoryTable.getSampleRatio();

        List<Integer> columnIndexes = columns.stream()
                .map(TestbedColumnHandle.class::cast)
                .map(TestbedColumnHandle::getColumnIndex).collect(toList());
        ...
        ConnectorPageSource pageSource = new DynamicFilteringPageSource(new FixedPageSource(pages), columns, dynamicFilter, enableLazyDynamicFiltering);
        QueryType queryType = memoryTable.getQueryType().orElse(null);
        if (queryType == QueryType.DELETE || queryType == QueryType.UPDATE) {
            return new TestbedUpdatablePageSource(pageSource);
        }
        return pageSource;
    }
